        ...
        @foreach ($activities as $activity)
            <div class="media media-list-view mb-5">
                ...
                <div class="media-body">
                    <h3>
                        <a class="text-warning" href="{{ route('teachers.activities.show',['id'=>$activity->id]) }}">
                            {{ $activity->title }}
                        </a>
                    </h3>
                    ...
                    <p class="show-read-more text-justify mb-lg-2">{{ $activity->description }}</p>
                        ...
                        <a href="{{ route('teachers.activities.show',['id'=>$activity->id]) }}" class="btn btn-sm btn-white text-uppercase mb-1 mr-2 btn-hover-warning w-25">
                            <i class="fa fa-eye mr-2" aria-hidden="true"></i> عرض الأسئلة
                        </a>
                    
                    
                        <a href="{{ route('teachers.activities.show.results',['id'=>$activity->id]) }}" class="btn btn-sm btn-white text-uppercase mb-1 mr-2 btn-hover-warning w-25">
